...
def main() -> None:
    ...
    required = ["Resistance", "x", "y", *predictors]
    ...
    metrics = {
        ...
        "predictors_file": str(predictors_path),
    ...
    (output_dir / "mgwr_metrics.json").write_text(
        json.dumps(metrics, indent=2),
        encoding="utf-8",
    )
    (output_dir / "mgwr_summary.txt").write_text(
        str(results.summary()),
        encoding="utf-8",
    )
    ...
    (output_dir / "mgwr_run_metadata.json").write_text(json.dumps(meta, indent=2), encoding="utf-8")

    print(
        json.dumps(
            {
                "mgwr_output_dir": str(output_dir),
                "mgwr_metrics": str(output_dir / "mgwr_metrics.json"),
            },
            ensure_ascii=False,
        )
    )
