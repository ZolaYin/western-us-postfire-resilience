...
INPUT = ROOT / "westernus_tcc_build_legacy_nlcd_20260411" / "westernus_current_candidate_table_plus_cbh_tcc.parquet"
BASELINE_METRICS = ROOT / "reduced_noevt_models_legacy_nlcd_20260411" / "reduced_model_metrics.csv"
OUT_DIR = ROOT / "westernus_rf_richer_oldcoast_partial_20260411"
OUT_METRICS = OUT_DIR / "westernus_rf_richer_oldcoast_partial_metrics.json"
OUT_IMPORTANCE = OUT_DIR / "westernus_rf_richer_oldcoast_partial_feature_importance.csv"
OUT_SAMPLE = OUT_DIR / "westernus_rf_richer_oldcoast_partial_sample.csv"
OUT_REPORT = OUT_DIR / "westernus_rf_richer_oldcoast_partial_report.md"
...
# Matches the old stronger Coast RF_noEVT_plusXY family as closely as current
# corrected Western US inputs allow. Road and trail density are still missing.
PREDICTORS = [
    "TS_elev_m_z",
    "TS_slope_deg_z",
    "TS_northness_z",
...
def main() -> None:
    ...
    metrics = {
        "model": "RF",
        "variant": "richer_oldcoast_partial_corrected_westernus",
        "input_table": str(INPUT),
        "rows_used": int(len(work)),
        "predictors": PREDICTORS,
        "missing_old_coast_predictors": MISSING_OLD_COAST_PREDICTORS,
    ...
    lines = [
        "# WesternUS Richer Old-Coast Partial RF",
        "",
        f"- Input table: `{INPUT}`",
        ...
        f"- Baseline corrected same16 RF test RMSE: `{metrics['baseline_same16_rf_test_rmse']:.6f}`",
        "",
        "This run matches the stronger old-Coast `RF_noEVT_plusXY` family as closely as current corrected WesternUS inputs allow.",
        "The exact old-Coast road and trail density predictors are still missing on the corrected WesternUS grid, so they were not included here.",
        "",
        "Predictors used:",
    ]
    lines.extend([f"- `{p}`" for p in PREDICTORS])
    lines.append("")
    lines.append("Missing old-Coast predictors:")
    lines.extend([f"- `{p}`" for p in MISSING_OLD_COAST_PREDICTORS])
