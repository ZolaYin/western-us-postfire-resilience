#!/usr/bin/env python3
"""Soft-MoE with LISA gate features — round 1.

Adds two new gate modes to the existing Soft-MoE framework:
  - lisa            : gate on LISA values only (z_i × mean_neighbor_z_i)
  - rawsmooth_lisa  : gate on raw + smooth + LISA features

Key transfer-safety property: LISA is computed within each CV fold using a
train-only KDTree, so no test-set information leaks into the gate.
"""
...
VARIANT_SPECS = [
    VariantSpec(
        ...
        "none",
        0,
        "M2 baseline RF (reference): base predictors + EVT group dummies.",
    ),
    VariantSpec(
        "m2_mgwr",
        "m2",
        ...
        "none",
        0,
        "M2+MGWR-smooth RF (parent of all soft experts): adds MGWR-radius smooth features.",
    ),
    VariantSpec(
        "m2_soft3_rawsmooth",
        "m2",
        ...
        "rawsmooth",
        3,
        "Soft-MoE (3 experts) over M2+MGWR; gate on raw + smooth stage5b gradients (round-1 baseline).",
    ),
    VariantSpec(
        "m2_soft3_lisa",
        "m2",
        ...
        "lisa",
        3,
        "Soft-MoE (3 experts) over M2+MGWR; gate on LISA values (z_i * mean-neighbor-z_i) of stage5b — spatially-aware, transfer-safe.",
    ),
    VariantSpec(
        "m2_soft3_rawsmooth_lisa",
        "m2",
        "m2_baseline",
        "m2_mgwr",
        "softmoe",
        ...
        "rawsmooth_lisa",
        3,
        "Soft-MoE (3 experts) over M2+MGWR; gate on raw + smooth + LISA stage5b features combined.",
    ),
]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Soft-MoE with LISA gate features for Western US block-transfer evaluation."
    )
    ...
    parser.add_argument("--lisa-k", type=int, default=LISA_K)
...
def build_smooth_bundle(
    ...
    return {"mgwr_train": matched_train, "mgwr_test": matched_test}


def compute_lisa_features(
    train_coords: np.ndarray,
    train_X_z: np.ndarray,
    test_coords: np.ndarray,
    test_X_z: np.ndarray,
    k: int = LISA_K,
) -> tuple[np.ndarray, np.ndarray]:
    """Compute LISA gate features: z_i * mean(z_j for j in k nearest train neighbors).

    Train LISA excludes the point itself (pure neighbor mean).
    Test LISA uses the train-only KDTree — no test neighbors, no leakage.
    """
    tree = KDTree(train_coords)

    k_tr = min(k + 1, len(train_coords))
    _, idx_tr = tree.query(train_coords, k=k_tr)
    idx_tr = idx_tr[:, 1:]  # exclude self
    local_mean_tr = train_X_z[idx_tr].mean(axis=1)  # (n_train, n_feat)
    lisa_train = (train_X_z * local_mean_tr).astype(np.float32)

    k_te = min(k, len(train_coords))
    _, idx_te = tree.query(test_coords, k=k_te)
    local_mean_te = train_X_z[idx_te].mean(axis=1)  # (n_test, n_feat)
    lisa_test = (test_X_z * local_mean_te).astype(np.float32)

    return lisa_train, lisa_test


def lisa_feature_names() -> list[str]:
    return [f"{pred}_lisa" for pred in STAGE5B_PREDS]
...
def main() -> None:
    ...
    group_array = (
        work[group_cols].to_numpy(dtype=np.float32) if group_cols else np.zeros((len(work), 0))
    )
    ...
    for split_name, is_test in split_masks.items():
        ...
        print("  Building smooth features ...", flush=True)
        ...
        print(f"  Computing LISA features (k={args.lisa_k}) ...", flush=True)
        lisa_train, lisa_test = compute_lisa_features(
            train_coords=coords[tr],
            train_X_z=X_map["stage5b"][tr],
            test_coords=coords[te],
            test_X_z=X_map["stage5b"][te],
            k=args.lisa_k,
        )
        ...
        gate_cache: dict[str, tuple[np.ndarray, np.ndarray, list[str]]] = {
            ...
            "rawsmooth_lisa": (
                ...
                list(STAGE5B_PREDS) + block_feature_names() + lisa_feature_names(),
            ),
        }
        ...
        for spec in variant_specs:
            ...
            gate_train_g, gate_test_g, gate_features = gate_cache[spec.gate_mode]
            metrics, imp_rows, diag_rows, center_rows, pred_df = run_softmoe_variant(
                spec=spec,
                X_train=X_train,
                X_test=X_test,
                feature_names=feature_names,
                gate_train=gate_train_g,
                gate_test=gate_test_g,
                gate_features=gate_features,
                y_train=y[tr],
                y_test=y[te],
    ...
    report_lines = [
        f"# Western US Soft-MoE LISA Gate Round 1 ({TODAY})",
        "",
        f"- Input: `{args.input}`",
        ...
        f"- LISA k neighbors: `{args.lisa_k}`",
        "",
        "## Variants",
        "",
    ]
