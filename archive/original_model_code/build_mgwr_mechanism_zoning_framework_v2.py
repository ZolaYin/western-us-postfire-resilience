#!/usr/bin/env python3
"""Build MGWR mechanism-based management zoning, framework v2.

This script reclassifies the complete MGWR EPA Level III ecoregion outputs into
the cleaner paper framework:
...
2. Use MGWR mechanism scores to define management type.
...
OUT_DIR = BASE / "resilience_management_zoning_epa_l3_framework_v2_2026-07-06"
...
MECHANISM_COLORS = {
    ...
    "Structure-dominated support": "#238b45",
    "Structure-dominated constraint": "#b35806",
    "Structure-dominated bidirectional": "#756bb1",
...
MANAGEMENT_COLORS = {
    ...
    "Structure-support management": "#238b45",
    "Structure-constraint management": "#b35806",
    "Structure-bidirectional management": "#756bb1",
    "Mixed-control transition": "#969696",
}
...
def prepare_scores(gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    out["forest_structure_influence"] = out[
        ["forest_structure_support", "forest_structure_constraint"]
    ].max(axis=1)
    ...
    score_table = pd.DataFrame(
        {
            "structure": out["forest_structure_influence"],
...
def mechanism_label(row: pd.Series) -> str:
    ...
    direction = str(row["forest_structure_direction"])
    if direction == "support":
        return "Structure-dominated support"
    if direction == "constraint":
        return "Structure-dominated constraint"
    return "Structure-dominated bidirectional"
...
def assign_for_threshold(gdf: gpd.GeoDataFrame, q: float) -> tuple[gpd.GeoDataFrame, float, float, float]:
    ...
    for _, row in out.iterrows():
        if row["resilience_priority_class"] == "High":
            ...
            elif mech == "Structure-dominated support":
                management.append("Structure-support management")
            elif mech == "Structure-dominated constraint":
                management.append("Structure-constraint management")
            elif mech == "Structure-dominated bidirectional":
                management.append("Structure-bidirectional management")
            else:
...
def assign_kmeans_clusters(gdf: gpd.GeoDataFrame, k: int = 4) -> tuple[gpd.GeoDataFrame, pd.DataFrame]:
    ...
    for cid, row in raw_centers.iterrows():
        ...
        if cid_int == low_cluster:
        ...
        elif row["forest_structure_support"] >= row["forest_structure_constraint"] * FOREST_DIRECTION_RATIO:
            label = "Structure-support cluster"
        elif row["forest_structure_constraint"] >= row["forest_structure_support"] * FOREST_DIRECTION_RATIO:
            ...
            label = "Structure-bidirectional cluster"
...
def save_diagnostic_panel(
    ...
    ax.set_axis_off()
    ax.set_title("A. Mechanism zones (q75)", fontsize=12, weight="bold")
    ...
    ax.bar(x - 0.18, sens["mixed_pct_pixels_mechanism"], width=0.36, color="#969696", label="Mechanism map")
...
def write_notes(
    ...
    lines = [
        "# MGWR mechanism zoning framework v2",
        "",
        "## Main choices",
        "",
        ...
        "- Forest structure handling: support and constraint are retained separately, but dominance uses `forest_structure_influence = max(support, constraint)`. The final label keeps the forest direction as support, constraint, or bidirectional.",
        ...
        "- If q75 leaves too much area in the mixed zone, use the sensitivity table to justify q70 or use the K-means cluster map as a data-driven alternative.",
    ...
    (OUT_DIR / "zoning_framework_v2_notes.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    for q in SENSITIVITY_QS:
        ...
        qtag = f"q{int(q * 100)}"
        csv_out = OUT_DIR / f"mgwr_mechanism_management_zones_{qtag}.csv"
        gpkg_out = OUT_DIR / f"mgwr_mechanism_management_zones_{qtag}.gpkg"
        gdf_save = gdf_q.copy()
        ...
        summarize_zones(gdf_q, mech_col).to_csv(
            OUT_DIR / f"mechanism_zone_summary_{qtag}.csv", index=False
        )
        summarize_zones(gdf_q, mgmt_col).to_csv(
            OUT_DIR / f"management_zone_summary_{qtag}.csv", index=False
        )
        save_crosstab(gdf_q, q, OUT_DIR / f"mechanism_by_priority_crosstab_{qtag}.csv")
    ...
    save_map(
        ...
        main_mech_col,
        MECHANISM_COLORS,
        "MGWR mechanism zones (q75 dominance threshold)",
        OUT_DIR / "fig_mgwr_mechanism_zones_q75",
    )
    save_map(
        main_gdf,
        main_mgmt_col,
        MANAGEMENT_COLORS,
        "MGWR mechanism-based management zones with resilience priority",
        OUT_DIR / "fig_mgwr_management_zones_q75",
    )
    save_diagnostic_panel(
        base,
        assigned,
        thresholds,
        OUT_DIR / "fig_mgwr_zoning_framework_v2_diagnostics",
    )
    ...
    clustered_save.drop(columns="geometry").to_csv(
        OUT_DIR / "kmeans4_mechanism_clusters.csv", index=False
    )
    clustered_save.to_file(OUT_DIR / "kmeans4_mechanism_clusters.gpkg", driver="GPKG")
    cluster_summary.to_csv(OUT_DIR / "kmeans4_mechanism_cluster_summary.csv", index=False)
    save_map(
        clustered,
        "kmeans4_mechanism_cluster",
        CLUSTER_COLORS,
        "Data-driven mechanism clusters (K-means, k=4)",
        OUT_DIR / "fig_kmeans4_mechanism_clusters",
    )
    ...
    metadata = {
        ...
        "forest_dominance_score": "max(forest_structure_support, forest_structure_constraint)",
    ...
    print(f"Wrote framework v2 zoning outputs to: {OUT_DIR}")
