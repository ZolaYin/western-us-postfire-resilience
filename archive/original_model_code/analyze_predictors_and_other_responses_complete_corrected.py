...
OUT_DIST_DIR = ROOT / "predictor_distribution_audit_complete_corrected_20260411"
OUT_RESP_DIR = ROOT / "other_response_rf_screen_complete_corrected_20260411"
OUT_NOXY_DIR = ROOT / "spatial_block_validation_no_xy_100km_20260411"

OUT_DIST_CSV = OUT_DIST_DIR / "predictor_distribution_audit.csv"
OUT_DIST_MD = OUT_DIST_DIR / "predictor_distribution_audit.md"
OUT_RESP_CSV = OUT_RESP_DIR / "other_response_rf_metrics.csv"
OUT_RESP_MD = OUT_RESP_DIR / "other_response_rf_report.md"
OUT_NOXY_JSON = OUT_NOXY_DIR / "spatial_block_validation_no_xy_100km_summary.json"
OUT_NOXY_MD = OUT_NOXY_DIR / "spatial_block_validation_no_xy_100km_report.md"
...
def summarize_series(series: pd.Series) -> dict[str, float]:
    ...
    return {
        ...
        "skew": float(pd.Series(valid).skew()),
        "zero_frac": float(np.mean(valid == 0)),
    }
...
def recommend_transform(raw_stats: dict[str, float], transform_type: str) -> str:
    ...
    if not np.isfinite(skew) or not np.isfinite(min_v):
        return "uncertain"
    if min_v >= 0 and skew >= 2:
        if max_v <= 100:
            return "right_skewed_bounded_or_percent_like_consider_log1p_or_sqrt_sensitivity"
        if zero_frac >= 0.05:
...
def run_group_holdout_rf(df: pd.DataFrame, response: str, predictors: list[str], block_km: int) -> dict:
    ...
    X = work[predictors]
    y = work[response]
    splitter = GroupShuffleSplit(n_splits=1, test_size=BLOCK_TEST_SIZE, random_state=RANDOM_STATE)
...
def main() -> None:
    ...
    OUT_RESP_DIR.mkdir(parents=True, exist_ok=True)
    OUT_NOXY_DIR.mkdir(parents=True, exist_ok=True)
    ...
    noxy_summary = {
        "response": RESPONSE,
        "predictor_set": "complete_corrected_without_xy",
        "predictors": NOXY_PREDICTORS,
        "random_split_no_xy": noxy_random,
        "group_holdout_100km_no_xy": noxy_block,
        "reference_random_split_with_xy_r2": float(complete_rf_metrics["test_r2"]),
        "reference_group_holdout_100km_with_xy_r2": float(
            spatial_xy_summary["group_holdout_20pct"]["r2"]
        ),
    }
    OUT_NOXY_JSON.write_text(json.dumps(noxy_summary, indent=2), encoding="utf-8")
    OUT_NOXY_MD.write_text(
        "\n".join(
            [
                "# Spatial Block Validation Without XY",
                "",
                f"- Input table: `{INPUT}`",
                f"- Response: `{RESPONSE}`",
                f"- Predictor count without `x/y`: `{len(NOXY_PREDICTORS)}`",
                f"- Random split without `x/y`: R2=`{noxy_random['test_r2']:.6f}`, RMSE=`{noxy_random['test_rmse']:.6f}`",
                f"- 100 km group holdout without `x/y`: R2=`{noxy_block['test_r2']:.6f}`, RMSE=`{noxy_block['test_rmse']:.6f}`",
                f"- Reference random split with `x/y`: R2=`{float(complete_rf_metrics['test_r2']):.6f}`",
                f"- Reference 100 km group holdout with `x/y`: R2=`{float(spatial_xy_summary['group_holdout_20pct']['r2']):.6f}`",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    for model_col, raw_col, transform_type in PREDICTOR_INFO:
        ...
        model_stats = summarize_series(df[model_col])
        row = {
            "model_predictor": model_col,
            "raw_source": raw_col,
            "current_transform": transform_type,
            "raw_n_valid": raw_stats["n_valid"],
            "raw_missing": raw_stats["missing"],
            ...
            "raw_zero_frac": raw_stats["zero_frac"],
            "model_skew": model_stats["skew"],
            "recommendation": recommend_transform(raw_stats, transform_type),
        }
        dist_rows.append(row)
    dist_df = pd.DataFrame(dist_rows)
    dist_df.to_csv(OUT_DIST_CSV, index=False)

    top_transform = dist_df.loc[
        dist_df["recommendation"].str.contains("candidate|consider|skew", na=False)
    ].copy()
    lines = [
        "# Predictor Distribution Audit",
        "",
        f"- Input table: `{INPUT}`",
        f"- Predictor count reviewed: `{len(dist_df)}`",
        "",
        "Highest raw skew predictors:",
    ]
    for _, row in dist_df.sort_values("raw_skew", ascending=False).head(10).iterrows():
        lines.append(
            f"- `{row['model_predictor']}` from `{row['raw_source']}`: raw_skew=`{row['raw_skew']:.3f}`, recommendation=`{row['recommendation']}`"
        )
    lines.extend(["", "Potential transformation review list:"])
    for _, row in top_transform.iterrows():
        lines.append(
            f"- `{row['model_predictor']}`: raw_skew=`{row['raw_skew']:.3f}`, zero_frac=`{row['raw_zero_frac']:.3f}`, recommendation=`{row['recommendation']}`"
        )
    OUT_DIST_MD.write_text("\n".join(lines) + "\n", encoding="utf-8")
    ...
    resp_df.to_csv(OUT_RESP_CSV, index=False)

    lines = [
        "# Other Response RF Screen On Complete Corrected Predictor Set",
        "",
        f"- Input table: `{INPUT}`",
        f"- Predictor count: `{len(COMPLETE_PREDICTORS)}`",
        f"- Train/test split: random `{int((1-TEST_SIZE)*100)}/{int(TEST_SIZE*100)}` with random_state `{RANDOM_STATE}`",
        "",
        "Responses ranked by RF test R2:",
    ]
    for _, row in resp_df.iterrows():
        lines.append(
            f"- `{row['response']}`: test_R2=`{row['test_r2']:.6f}`, test_RMSE=`{row['test_rmse']:.6f}`, rows=`{int(row['rows_used'])}`"
        )
    OUT_RESP_MD.write_text("\n".join(lines) + "\n", encoding="utf-8")

    print(
        json.dumps(
            {
                "noxy_summary": str(OUT_NOXY_JSON),
                "distribution_csv": str(OUT_DIST_CSV),
                "other_response_csv": str(OUT_RESP_CSV),
            },
            ensure_ascii=False,
        )
    )
