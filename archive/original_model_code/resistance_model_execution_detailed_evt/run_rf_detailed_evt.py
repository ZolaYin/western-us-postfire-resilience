"""
Random Forest script for the detailed EVT Resistance pathway.
"""
...
def main():
    ...
    imp = pd.DataFrame({"predictor": predictors, "importance": model.feature_importances_}).sort_values(
        "importance", ascending=False
    )
    imp.to_csv(out_dir / "rf_detailed_evt_feature_importance.csv", index=False)
    ...
    (out_dir / "rf_detailed_evt_metrics.json").write_text(json.dumps(metrics, indent=2))
