...
def compute_moran(df: pd.DataFrame, residuals: np.ndarray) -> dict:
    ...
    w.transform = "R"
    m = Moran(residuals.astype(float), w, permutations=0)
    return {"moran_i": float(m.I), "z_norm": float(m.z_norm), "p_norm": float(m.p_norm)}
    ...
    rows.append(
        {
            ...
            **compute_moran(work, (y - ols_pred).to_numpy()),
        }
    )
    ...
    rows.append(
        {
            ...
            **compute_moran(work, (y - rf_pred).to_numpy()),
        }
    )
    ...
    rows.append(
        {
            ...
            **compute_moran(work, (y - xgb_pred).to_numpy()),
        }
    )
    return rows
...
def main() -> None:
    ...
    for thr in THRESHOLDS:
        ...
        print(f"[start] {label} rows={len(sub)}", flush=True)
        if len(sub) == 0:
            ...
            print(f"[done] {label} empty", flush=True)
            continue
        ...
        pd.DataFrame(dist_rows).to_csv(OUT_DIR / "bi_threshold_distribution_summary.csv", index=False)
        pd.DataFrame(model_rows).to_csv(OUT_DIR / "bi_threshold_model_summary.csv", index=False)
        print(f"[done] {label} models={sum(1 for r in model_rows if r['threshold'] == thr)}", flush=True)
    ...
    report_lines = [
        ...
        "- This means the current `Bi` values are almost always far below those cutoffs.",
    ]
