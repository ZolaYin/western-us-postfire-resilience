...
OUT_DIR = BASE_DIR / "rf_spatial_terms_recovery_y_2026-03-31"
RESPONSES = ["IRI_good_10yr", "STAB_10yr"]
RANDOM_STATE = 42
MORAN_K = 8
...
def main() -> None:
    ...
    summary.to_csv(OUT_DIR / "rf_spatial_terms_recovery_y_summary.csv", index=False)
    lines = ["RF spatial-term comparison for recovery y"]
    for _, row in summary.iterrows():
        lines.append(
            f"{row['response']} | {row['variant']}: r2={row['test_r2']:.4f}, "
            f"rmse={row['test_rmse']:.4f}, moran_i={row['moran_i']:.4f}"
        )
    (OUT_DIR / "rf_spatial_terms_recovery_y_summary.txt").write_text("\n".join(lines))
