...
DEFAULT_INPUT = Path(
    "/content/drive/MyDrive/US_Fire_and_Ecology_Data/WUS_1km/"
    "resistance_model_execution_near_t0_aggregated/MGWR_ready_table_near_t0_aggregated.parquet"
)
DEFAULT_PREDICTORS = Path(
    "/content/drive/MyDrive/US_Fire_and_Ecology_Data/WUS_1km/"
    "grouped_gwr_execution_package_2026-03-30/predictors_B_topo_soil_plus_forest.txt"
)
...
def main():
    ...
    parser.add_argument("--fixed-bandwidth", type=int, default=None)
    ...
    meta = {
        ...
        "localR2_mean": float(results.localR2.mean()) if hasattr(results, "localR2") else None,
        "localR2_min": float(results.localR2.min()) if hasattr(results, "localR2") else None,
        "localR2_max": float(results.localR2.max()) if hasattr(results, "localR2") else None,
    }
