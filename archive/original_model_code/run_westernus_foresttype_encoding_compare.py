...
OUT_DIR = ROOT / f"foresttype_encoding_compare_{TODAY}"
...
def fit_and_score(df: pd.DataFrame, response: str, predictors: list[str]) -> list[dict]:
    ...
    work = df[[c for c in needed if c in df.columns]].replace([np.inf, -np.inf], np.nan).dropna().copy()
...
def main() -> None:
    ...
    variants = {
        "baseline_only": BASE_PREDS,
        "evt_proxy_2": BASE_PREDS + EVT_PROXY_PREDS,
        "evt_group_class_5plusunknown": BASE_PREDS + group_cols,
        "evt_raw_code_all": BASE_PREDS + code_cols,
    }
    ...
    out_df = pd.DataFrame(all_rows)[
        ["variant", "split", "rows", "n_predictors", "r2", "rmse"]
    ].sort_values(["split", "r2"], ascending=[True, False])
    out_df.to_csv(OUT_DIR / "foresttype_encoding_compare_metrics.csv", index=False)

    lines = [
        f"# Forest-Type Representation Comparison ({TODAY})",
        "",
        "Response: Resistance",
        "",
        "| Variant | Split | Rows | Predictors | R2 | RMSE |",
        "|---|---|---:|---:|---:|---:|",
    ]
    for _, r in out_df.iterrows():
        lines.append(
            f"| {r['variant']} | {r['split']} | {int(r['rows'])} | {int(r['n_predictors'])} | {r['r2']:.4f} | {r['rmse']:.4f} |"
        )
    lines.extend(
        [
            "",
            "Variant definitions:",
            f"- baseline_only: {len(BASE_PREDS)} baseline predictors, no forest-type term.",
            "- evt_proxy_2: baseline + two EVT-derived proxy variables.",
            f"- evt_group_class_5plusunknown: baseline + {len(group_cols)} broad forest-type dummy columns "
            "(conifer / deciduous / mixed / shrub / nonwoody / unknown).",
            f"- evt_raw_code_all: baseline + {len(code_cols)} raw EVT code dummy columns.",
        ]
    )
    (OUT_DIR / "foresttype_encoding_compare_report.md").write_text("\n".join(lines))
